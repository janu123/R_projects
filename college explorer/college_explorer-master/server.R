...
shinyServer(function(input, output, session) {
  ...
  output$map <- renderLeaflet({leaflet() %>% 
      setView(lng = 78.685277,lat = 10.8055, zoom = 12) %>% 
      addTiles()  
  })
  ...
  zipsInBounds <- reactive({
    zipdata[zipdata$longitude >= input$range[1] & zipdata$latitude <= input$range[2],]
  ...
  session$onFlushed(once=TRUE, function() {
    paintObs <- observe({
      ...
      for (from in seq.int(1, nrow(zipdata), chunksize)) {
        ...
        # before we get here
          leafletProxy("map", data = zipsInBounds()) %>%
            clearShapes() %>%
            addCircles(
